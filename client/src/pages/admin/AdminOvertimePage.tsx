...
import { 
  Plus, Calendar, Clock, User as UserIcon, Eye, Printer, Trash2, Check, X, FileText, Send, Upload, ArrowLeft, Image as ImageIcon, CheckCircle, ShieldCheck
} from "lucide-react";
...
export default function AdminOvertimePage() {
  ...
  // Modal State for Detail / Inspection View
  ...
  // Dummy Overtime Data List (Langsung Bekerja Tanpa Persetujuan Awal Admin)
  const [overtimeRequests, setOvertimeRequests] = useState([
    {
      ...
      status: "verified", // verified = SELESAI & TELAH DIPERIKSA ADMIN
    ...
    },
    {
      ...
      status: "ongoing", // ongoing = BERLANGSUNG LANGSUNG TANPA TUNGGU ADMIN
    ...
    },
    {
      ...
      status: "completed_unverified", // completed_unverified = SELESAI, SIAP DIPERIKSA ADMIN
      reason: "Patroli pengawasan ekstra pengiriman barang malam hari",
      finalDesc: "Pengawasan barang malam selesai aman 100%, seluruh pintu gudang terkunci",
  ...
  const handleCreateAssignment = (e: React.FormEvent) => {
    ...
    const newOvertime = {
      ...
      duration: "3 Jam 30 Menit",
      status: "ongoing", // Langsung aktif berjalan di HP Karyawan
      reason: assignTask,
      finalDesc: "-",
    ...
    toast({
      ...
      description: `Surat Perintah Lembur (SPL) telah terkirim. Karyawan dapat langsung bekerja tanpa menunggu persetujuan.`,
    });
  };

  const handleVerifyReport = (id: number) => {
    setOvertimeRequests(prev => prev.map(item => item.id === id ? { ...item, status: "verified" } : item));
    setViewDetail(null);
    toast({
      title: "Laporan Lembur Diverifikasi!",
      description: `Surat SPL & Laporan Pekerjaan lembur #${id} telah disetujui & diverifikasi oleh Admin.`,
    });
  };
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <table>
          ...
          <tr><td class="label">Status Verifikasi Admin</td><td class="colon">:</td><td><strong>${item.status === 'verified' ? 'TELAH DIPERIKSA & VERIFIED' : item.status === 'ongoing' ? 'BERLANGSUNG' : 'SELESAI (SIAP DIPERIKSA ADMIN)'}</strong></td></tr>
        </table>
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Karyawan dapat <strong className="text-gray-800">langsung mulai lembur tanpa menunggu persetujuan awal Admin</strong>. Admin memeriksa Surat SPL & Hasil Foto Pekerjaan setelah selesai.
          </p>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Status lembur aktif & laporan pekerjaan yang siap diperiksa Admin.</p>
          </div>
              ...
              <table className="w-full text-sm text-left">
                <thead className="text-[10px] text-gray-400 font-black uppercase tracking-widest bg-gray-50/50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-center">Pemeriksaan Admin</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-50">
                  {sortedRequests.map((req) => (
                    <tr key={req.id} className="hover:bg-gray-50/50 transition-colors">
                        ...
                        <span
                          className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg border whitespace-nowrap flex items-center gap-1 w-fit ${
                            req.status === "verified"
                              ...
                              : req.status === "completed_unverified"
                              ? "text-blue-700 bg-blue-50 border-blue-200"
                              : "text-orange-700 bg-orange-50 border-orange-200 animate-pulse"
                          }`}
                        >
                          {req.status === "verified" ? (
                            <><ShieldCheck className="w-3 h-3 text-emerald-600" /> Diperiksa Admin</>
                          ) : req.status === "completed_unverified" ? (
                            <><Clock className="w-3 h-3 text-blue-600" /> Selesai (Periksa Laporan)</>
                          ) : (
                      ...
                      {/* Column 6: Aksi Pemeriksaan Admin */}
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-center gap-1.5">
                          {/* Eye Detail & Inspection Button */}
                          <Button
                            variant={req.status === "completed_unverified" ? "default" : "outline"}
                            size="sm"
                            className={`rounded-lg h-8 px-3 gap-1 text-xs font-bold ${
                              req.status === "completed_unverified" 
                                ? "bg-blue-600 hover:bg-blue-700 text-white shadow-xs" 
                                : "text-blue-600 border-blue-100 hover:bg-blue-50"
                            }`}
                            onClick={() => setViewDetail(req)}
                            title="Periksa Surat SPL & Bukti Foto Pekerjaan"
                          >
                            <Eye className="w-3.5 h-3.5" />
                            <span>{req.status === "completed_unverified" ? "Periksa Laporan" : "Detail"}</span>
                          </Button>
        ...
        <DialogContent className="sm:max-w-xl rounded-2xl p-6 bg-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs text-gray-500">
              Isi instruksi penugasan lembur. Surat SPL otomatis terkirim dan <strong className="text-gray-700">karyawan dapat langsung bekerja tanpa menunggu konfirmasi</strong>.
            </DialogDescription>
          </DialogHeader>
      ...
      {/* ========================================================================= */}
      {/* MODAL POPUP 2: PERIKSA SURAT & LAPORAN HASIL PEKERJAAN                   */}
      {/* ========================================================================= */}
            ...
            <div>
              <DialogHeader className="border-b pb-3 mb-4">
                <DialogTitle className="text-lg font-black text-gray-900 flex items-center justify-between">
                  <span>Pemeriksaan Surat SPL & Laporan Lembur</span>
                  <span className={`text-[10px] px-2.5 py-1 rounded-lg border uppercase ${
                    viewDetail.status === 'verified' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'
                  }`}>
                    {viewDetail.status === 'verified' ? 'Telah Diperiksa & Verified' : 'Siap Diperiksa Admin'}
                  </span>
                </DialogTitle>
                <DialogDescription className="text-xs text-gray-500">
              ...
              <div className="space-y-4 text-xs text-gray-700">
                ...
                {viewDetail.finalDesc && viewDetail.finalDesc !== "-" && (
                  <div>
              ...
              <DialogFooter className="mt-6 pt-3 border-t flex flex-row items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  {viewDetail.status !== "verified" && (
                    <Button 
                      onClick={() => handleVerifyReport(viewDetail.id)} 
                      className="rounded-xl text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white gap-2"
                    >
                      <CheckCircle className="w-4 h-4" /> Tandai Berkas Selesai Diperiksa
                    </Button>
                  )}
